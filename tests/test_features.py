...
class IngestionFeatureTests(unittest.TestCase):
    ...
    def test_build_window_metrics_aggregates_security_and_llm_signals(self) -> None:
        ...
        self.assertEqual(metrics["observed_log_types"], ["ERROR", "LLM_TIMEOUT", "SECURITY"])
        self.assertEqual(metrics["observed_client_ips"], ["10.0.0.1"])
        self.assertEqual(metrics["observed_service_ids"], ["svc-a", "svc-b"])
        self.assertEqual(metrics["observed_http_status_codes"], ["403", "500"])
        self.assertEqual(metrics["observed_llm_model_ids"], ["gpt-4.1"])
